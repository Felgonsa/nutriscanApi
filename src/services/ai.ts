...
export async function getMealDetailsFromText({
  ...
  log('text', text); log('createdAt', createdAt);
...
type GetMealDetailsFromImage = {
  imageURL: string;
  createdAt: Date;
}

export async function getMealDetailsFromImage({ imageURL, createdAt }: GetMealDetailsFromImage) {
  ...
  try {
    response = await client.chat.completions.create({
      ...
      messages: [
        {
          ...
          Considere proporções e volume visível para estimar a quantidade. Quando houver incerteza sobre o tipo exato do alimento (por exemplo, tipo de arroz, corte de carne), 
          use o tipo mais comum. Seja direto, objetivo e evite explicações. Apenas retorne os dados em JSON no formato abaixo:
          Apenas retorne JSON válido, **sem usar blocos de código** como \`\`\`.
        ...
        },
      {
        role: 'user',
        content: [
          { type: 'text', text: "Aqui está a imagem da refeição:" },
